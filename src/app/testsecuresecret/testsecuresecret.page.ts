...
@Component({
  selector: 'app-testsecuresecret',
  templateUrl: 'testsecuresecret.page.html',
  styleUrls: ['testsecuresecret.page.scss']
})
export class TestSecureSecretPage implements AfterViewInit {
  ...
  async ngAfterViewInit() {
    ...
    console.log("%%%% in TestSecureSecretPage this._sqlite " + this._sqlite)
    try {
      await this.runTest();
...
//      if(!(await this._sqlite.isSecretStored()).result) {
        await this._sqlite.setEncryptionSecret('abbey clammy gird night test');
//      } else {
//        await this.showAlert("Secure Secret already been set");
//      }
